...
class amberalertapp extends Homey.App {
    log() {
    ...
    onInit() {
        ...
        this.feedUrl = 'https://services.burgernetcdn.nl/landactiehost/rss';
    ...
    async checkRssFeed() {
        try {
            ...
            if (feed && feed.items && feed.items.length) {
                ...
                const { title, link, description, DescriptionExt, msgid, PublishDate, url, circle, area  } = latestItem;
                const data = {
                    title,
                    link,
                    description,
